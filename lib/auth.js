...
// API URL for token refresh endpoint
...
/**
 * Store authentication token in localStorage and cookie (for middleware)
...
export function setAuthToken(token) {
  try {
    ...
    // Also set cookie for Next.js middleware (server-side route protection)
    // Cookie is httpOnly=false so it can be read by middleware
    // Set SameSite=Lax for security while allowing normal navigation
...
export async function ensureValidToken() {
  ...
  // If token is expired, can't refresh
  if (isTokenExpired(token)) {
    clearAuthToken();
    return false;
  }
  ...
  if (isTokenNearExpiration(token)) {
    ...
    if (!refreshed) {
      // Token still valid for now, just warn
      console.warn('Token refresh failed, token will expire soon');
    }
  }
