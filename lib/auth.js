...
export async function isAuthenticated() {
  try {
    ...
    if (isTokenExpired(token)) {
      // Automatically clear expired token from localStorage
      clearAuthToken();
      return false;
    }
...
export async function getAuthHeaders() {
  ...
  // Validate token hasn't expired before returning it
  if (isTokenExpired(token)) {
    // Clear expired token
    clearAuthToken();
    return {};
...
export async function isTokenNearExpiry(thresholdSeconds = NEAR_EXPIRATION_THRESHOLD) {
  try {
    ...
    if (!token) {
      return false; // No token means not near expiry, just not authenticated
    }
