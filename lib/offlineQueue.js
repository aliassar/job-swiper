...
class OfflineQueue {
  constructor() {
    ...
    // CRITICAL: Listen for online event to process queue when connectivity is restored
    if (typeof window !== 'undefined') {
      window.addEventListener('online', () => {
        console.log('[OfflineQueue] Connection restored - processing pending operations');
    ...
    // Also try to process queue on startup if online
    if (typeof navigator !== 'undefined' && navigator.onLine && this.queue.length > 0) {
      console.log('[OfflineQueue] Online at startup with pending queue - processing');
