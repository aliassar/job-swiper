...
class OfflineQueue {
  ...
  async processQueue() {
    ...
    try {
      // Sort queue by timestamp to ensure actions are replayed in correct order
      ...
      while (this.queue.length > 0) {
        ...
        try {
          // Execute the API call with idempotency key in headers
          const payloadWithIdempotency = {
            ...operation.payload,
            idempotencyKey: operation.idempotencyKey,
          };
          
          await apiCall(payloadWithIdempotency);
          ...
          if (operation.retries >= MAX_RETRIES) {
            ...
            const backoffDelay = Math.min(Math.pow(2, operation.retries) * 1000, MAX_BACKOFF_DELAY);
            console.log(`Retrying ${operation.type} in ${backoffDelay / 1000}s (attempt ${operation.retries + 1}/${MAX_RETRIES})`);
      ...
      // Process them in the next event loop iteration to avoid blocking
      // 
      // Note: setTimeout(0) is used here intentionally:
      // - Only scheduled when queue has items (not continuous)
      // - The this.processing guard at function start prevents concurrent execution
      // - This ensures new items added during processing are handled
      // - Alternative approaches (e.g., while loop) would block the main thread
      // 
      // This pattern is safe because:
      // 1. The processing flag prevents multiple simultaneous processQueue calls
      // 2. setTimeout queues the call for next event loop, not recursively
      // 3. If queue is empty when setTimeout fires, function returns immediately
      if (this.queue.length > 0) {
