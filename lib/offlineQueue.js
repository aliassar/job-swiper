...
import { MAX_QUEUE_RETRIES, QUEUE_SAVE_DEBOUNCE_DELAY } from './constants';
...
class OfflineQueue {
  ...
  getOppositeOperation(type, payload) {
    // For saveJob toggle operations, opposite is when favorite state is different
    if (type === 'saveJob') {
      return {
        type: 'saveJob',
        // If current is saving (favorite: true), opposite is unsaving (favorite: false)
        matchPayload: (p) => p.favorite === !payload.favorite
      };
    }
    return null;
  }
