...
class OfflineQueue {
  ...
  async processQueue() {
    ...
    try {
      ...
      // Schedule next run if queue has items
      // 
      // Using setTimeout(0) for event loop scheduling:
      // - Only scheduled when queue has items (not continuous)
      // - The this.processing guard prevents concurrent execution
      // - Ensures items added during processing are handled
      // - setTimeout queues for next event loop, not recursive call stack
      if (this.queue.length > 0) {
        setTimeout(() => this.processQueue(), 0);
      }
    }
