...
export function useSwipeJobs(currentIndex = 0) {
    ...
    const shouldPrefetch = remainingJobs <= 10 && hasMore && !isValidating;

    // Auto-fetch more jobs when approaching the end (10 or fewer cards remaining)
    useEffect(() => {
        if (shouldPrefetch) {
            console.log(`[useSwipeJobs] Auto-fetching: ${remainingJobs} jobs remaining`);
            setSize(size + 1);
        }
    }, [shouldPrefetch, remainingJobs, size, setSize]);
