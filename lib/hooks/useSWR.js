...
export function useSavedJobsInfinite(search = '') {
  ...
  // Merge server data with local pending sync jobs
  let savedJobs;
  if (isOnline && data) {
    const serverJobs = data.flatMap(page => page.saveds || []);
    // Get locally saved jobs that are pending sync (not yet on server)
    const pendingJobs = (cachedSavedJobs || []).filter(job => job.pendingSync);
    // Merge: pending jobs first, then server jobs (avoid duplicates by id)
    const serverJobIds = new Set(serverJobs.map(j => j.id));
    const uniquePendingJobs = pendingJobs.filter(j => !serverJobIds.has(j.id));
    savedJobs = [...uniquePendingJobs, ...serverJobs];
  } else {
...
export function useApplicationsInfinite(search = '') {
  ...
  const getKey = (pageIndex, previousPageData) => {
    ...
    if (previousPageData && !previousPageData.hasMore) return null;
    const searchParam = search ? `&search=${encodeURIComponent(search)}` : '';
    return `/api/applications?page=${pageIndex + 1}&limit=${ITEMS_PER_PAGE}${searchParam}`;
  };
