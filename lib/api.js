...
export const applicationsApi = {
  // Get application counts by category
  getApplicationCounts: () => http.get('/api/applications/counts'),
  ...
  // Archive/unarchive an application
  archiveApplication: (applicationId) => http.post(`/api/applications/${applicationId}/archive`),

  // Get archived applications
  getArchivedApplications: (search = '') => {
    const params = search ? `?search=${encodeURIComponent(search)}` : '';
    return http.get(`/api/applications/archived${params}`);
  },

  // Save/unsave an application for later
  saveForLater: (applicationId) => http.post(`/api/applications/${applicationId}/save-for-later`),

  // Get saved-for-later applications
  getSavedForLaterApplications: (search = '') => {
    const params = search ? `?search=${encodeURIComponent(search)}` : '';
    return http.get(`/api/applications/saved-for-later${params}`);
  },

  // Bulk actions
  bulkDelete: (ids) => http.post('/api/applications/bulk/delete', { ids }),
  bulkRegenerate: (ids) => http.post('/api/applications/bulk/regenerate', { ids }),
  bulkArchive: (ids) => http.post('/api/applications/bulk/archive', { ids }),
  bulkSaveForLater: (ids) => http.post('/api/applications/bulk/save-for-later', { ids }),
};
...
// Custom Job API calls
export const customJobApi = {
  submitJob: (text, applyLink) => http.post('/api/custom-job', { text, apply_link: applyLink }),
};
