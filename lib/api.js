...
async function apiRequest(endpoint, options = {}) {
  ...
  const headers = isFormData && options.headers && Object.keys(options.headers).length === 0
    ? undefined
    : {
...
export const applicationsApi = {
  ...
  // Verify/reject/rollback CV documents (legacy method for backward compatibility)
  verifyDocuments: (applicationId, action, timestamp = null) => apiRequest(`/api/applications/${applicationId}/verify`, {
    method: 'POST',
    body: JSON.stringify({ action, timestamp }),
  }),
  
  // Handle message verification and sending (legacy method for backward compatibility)
  handleMessage: (applicationId, action, timestamp = null, message = null) => apiRequest(`/api/applications/${applicationId}/message`, {
    method: 'POST',
    body: JSON.stringify({ action, timestamp, message }),
  }),
...
export const notificationsApi = {
  ...
  getNotifications: (page = 1, limit = 20) => {
    return apiRequest(`/api/notifications?page=${page}&limit=${limit}`);
  },
