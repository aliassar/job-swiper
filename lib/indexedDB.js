    ...
    this.initPromise = new Promise((resolve, reject) => {
      ...
      request.onsuccess = () => {
        ...
        if (!db.objectStoreNames.contains(STORE_NAME)) {
          console.warn('IndexedDB object store missing, recreating database...');
          db.close();
          this.db = null;
          this.initPromise = null;
          ...
          deleteRequest.onsuccess = () => {
            console.log('Corrupted IndexedDB deleted, reinitializing...');
            // Recursively reinitialize
            this.init().then(resolve).catch(reject);
          };
          deleteRequest.onerror = () => {
            console.error('Failed to delete corrupted IndexedDB:', deleteRequest.error);
